...
import com.github.xniter.customenderchest.utils.EncodingUtil;
...
public class MysqlStorage implements StorageInterface {
	...
	public MysqlStorage(CustomEnderChest enderchest) {
	...
	public boolean hasDataFile(UUID player) {
		...
		try {	 
			...
			CustomEnderChest.log.severe("Encountered SQL Exception: " + e.getMessage());
	      } finally {
				try {
					if (result != null) {
						result.close();
					...
					CustomEnderChest.log.severe("Encountered an Exception: " + e.getMessage());
				}
		  }
	      return false;
	}
		...
		if (enderchest.getModdedSerializer() != null) {
			try {
				...
				try {
					...
					CustomEnderChest.log.severe("Failed to decode inventory for " + playerName + "! Error: " + ex.getMessage());
					ex.printStackTrace();
				}
			}
		} else {
			try {
				...
				CustomEnderChest.log.severe("Failed to decode inventory for " + playerName + "! Error: " + e.getMessage());
				//TODO
	...
	private String encodeInventory(Inventory inv, String playerName) {
		if (enderchest.getModdedSerializer() != null) {
			try {
				...
				CustomEnderChest.log.severe("Failed to save enderchest data for " + playerName + "! Error: " + e.getMessage());
				e.printStackTrace();
			}
		} else {
			try {
