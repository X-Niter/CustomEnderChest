...
public class EnderChestUtils {
	...
	public EnderChestUtils(CustomEnderChest enderchest) {
    ...
    public void openMenu(Player p) {
		...
		if (inv == null) {
			...
			if (size > inv.getSize()) {
				...
				Bukkit.getScheduler().runTaskAsynchronously(enderchest, () -> {
					if (enderchest.getStorageInterface().hasDataFile(p.getUniqueId())) {
						Inventory eInv = Bukkit.getServer().createInventory(p, size, enderChestTitle);
						// Set the loaded inventory back to the player's data
						enderchest.getDataHandler().setData(p.getUniqueId(), eInv);

						// Open the inventory on the main thread
						Bukkit.getScheduler().runTask(enderchest, () -> p.openInventory(eInv));
					}
				});
			} else {
